...
#class AttU_Net(nn.Module):
#    def __init__(self, img_ch=3, output_ch=1, chs=[64, 128, 256, 512, 1024]):
#        super(AttU_Net, self).__init__()
#        chs = chs
#        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
#
#        self.Conv1 = conv_block(ch_in=img_ch, ch_out=chs[0])
#        self.Conv2 = conv_block(ch_in=chs[0], ch_out=chs[1])
#        self.Conv3 = conv_block(ch_in=chs[1], ch_out=chs[2])
#        self.Conv4 = conv_block(ch_in=chs[2], ch_out=chs[3])
#        self.Conv5 = conv_block(ch_in=chs[3], ch_out=chs[4])
#
#        self.Up5 = up_conv(ch_in=chs[4], ch_out=chs[3])
#        self.Att5 = Attention_block(F_g=chs[3], F_l=chs[3], F_int=chs[3])
#        self.Up_conv5 = conv_block(ch_in=chs[4], ch_out=chs[3])
#
#        self.Up4 = up_conv(ch_in=chs[3], ch_out=chs[2])
#        self.Att4 = Attention_block(F_g=chs[2], F_l=chs[2], F_int=chs[1])
#        self.Up_conv4 = conv_block(ch_in=chs[3], ch_out=chs[2])
#
#        self.Up3 = up_conv(ch_in=chs[2], ch_out=chs[1])
#        self.Att3 = Attention_block(F_g=chs[1], F_l=chs[1], F_int=chs[0])
#        self.Up_conv3 = conv_block(ch_in=chs[2], ch_out=chs[1])
#
#        self.Up2 = up_conv(ch_in=chs[1], ch_out=chs[0])
#        self.Att2 = Attention_block(F_g=chs[0], F_l=chs[0], F_int=32)
#        self.Up_conv2 = conv_block(ch_in=chs[1], ch_out=chs[0])
#
#        self.Conv_1x1 = nn.Conv2d(chs[0], output_ch, kernel_size=1, stride=1, padding=0)
#
#    def forward(self, x):
#        # encoding path
#        x1 = self.Conv1(x)
