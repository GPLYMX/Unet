...
def get_impurity_mask(img, model):
    """
    读取十三通道图片，生成杂质的掩码图
    :param img_root:
    :return:
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    print("模型加载时间：", t1 - t0)
    roots = r'G:\datas\data\fangshuijingmai\example_videos'
    save_root = r'G:\datas\data\fangshuijingmai\preds'
    save_fps = 20  # 设置保存的视频帧率
    for i in os.listdir(roots):
        if i.endswith(('bmp', 'tif', 'avi')):
            # 读取视频
            video_path = os.path.join(roots, i)

            cap = cv2.VideoCapture(video_path)
            frame_width = int(cap.get(3))  # 获取视频宽度
            frame_height = int(cap.get(4))  # 获取视频高度
            fps = int(cap.get(cv2.CAP_PROP_FPS))  # 原始视频帧率
            fourcc = cv2.VideoWriter_fourcc(*'XVID')  # 编码格式
            out = cv2.VideoWriter(os.path.join(save_root, i), fourcc, save_fps, (frame_width, frame_height))
            ...
            while cap.isOpened():
                ...
                img_pil = Image.fromarray(image)  # 转换为PIL格式
                img_tensor = transform(img_pil)  # 转换为Tensor格式

                # 计算掩码
                mask = get_impurity_mask(img_tensor, model)  # 这里的 model 需要事先定义

                # 生成红色掩码并叠加
                color_mask = np.zeros_like(image)
                color_mask[mask > 0] = [255, 0, 255]  # 设为红色
                overlay = cv2.addWeighted(image, 1, color_mask, 0.5, 0)

                # 显示处理时间
                print("计算杂质掩码所需总时间", time.time() - t0)

                # 转换回 BGR 以适配 OpenCV 写入视频
                overlay_bgr = cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR)
                out.write(overlay_bgr)  # 写入新视频

            cap.release()
            out.release()
            cv2.destroyAllWindows()
            print("视频处理完成，已保存到", os.path.join(save_root, i))
            ...
            # img = transform(img)
            # mask = get_impurity_mask(img, model)
            # color_mask = np.zeros_like(image)
